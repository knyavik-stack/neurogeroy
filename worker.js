...
async function saveResult(){const initData=window.Telegram?.WebApp?.initData;if(!initData)return{ok:false,error:"Telegram authorization unavailable"};try{const r=await fetch("/api/game-sessions/lightning",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({initData,reaction_ms:lastReaction,difficulty,false_starts:falseStarts})});return await r.json()}catch(e){return{ok:false,error:"Network error"}}}
function render(){
