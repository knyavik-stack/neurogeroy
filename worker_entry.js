...
const GAME_CATALOG = [
  {code:"lightning",title:"Молния",skill_domain:"reaction",route:"/",sort_order:10,config:{}},
  {code:"memory_grid",title:"Память-сетка",skill_domain:"working_memory",route:"/games/memory-grid",sort_order:20,config:{}},
  {code:"switcher",title:"Переключатель",skill_domain:"cognitive_flexibility",route:"/games/switcher",sort_order:30,config:{}},
  {code:"focus_ribbon",title:"Фокус-лента",skill_domain:"sustained_attention",route:"/games/focus-ribbon",sort_order:40,config:{}}
];
  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === "POST" && url.pathname === "/api/game-sessions") {
      try {
        ...
        if(gridSize!==null&&(!Number.isInteger(gridSize)||gridSize<2||gridSize>10)||patternLength!==null&&(!Number.isInteger(patternLength)||patternLength<1||patternLength>100)||inputTiming!==null&&(!Array.isArray(inputTiming)||inputTiming.length>100)||metadata===null||typeof metadata!=="object"||Array.isArray(metadata)) return json({ok:false,error:"Invalid game metrics"},400);
...
async function listGames(env){
  ...
  if(!env.SUPABASE_URL||!key)return json({ok:true,games:GAME_CATALOG});
  try{
    ...
    if(!r.ok)return json({ok:true,games:GAME_CATALOG});
    const games=await r.json();
    return json({ok:true,games:Array.isArray(games)&&games.length?games:GAME_CATALOG});
  }catch{return json({ok:true,games:GAME_CATALOG})}
}

async function gameEnabled(env,code){
  const key=env.SUPABASE_SECRET_KEY;
  if(!env.SUPABASE_URL||!key)return GAME_CATALOG.some(g=>g.code===code);
  try{
