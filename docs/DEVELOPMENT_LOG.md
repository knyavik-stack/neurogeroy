...
- Удалён импорт и отдельный runtime-screen `games_page.js` из канонического роутера; файл оставлен в репозитории как исторический UI-артефакт, но production route `/games` его больше не рендерит.
