...
export default{async fetch(request,env,ctx){
  ...
  if(request.method==="GET"&&u.pathname==="/")return new Response(enhanceHtml(menuHtml()),{headers:htmlHeaders()});
...
function menuHtml(){
  const cards=CATALOG.map(g=>'<button class="game" data-route="'+g.route+'"><span>'+g.icon+'</span><b>'+esc(g.title)+'</b></button>').join("");
