    ...
    if (request.method === "POST" && url.pathname === "/api/game-sessions") {
      try {
        ...
        const rpc=await fetch(env.SUPABASE_URL.replace(/\/$/,"")+"/rest/v1/rpc/record_game_session",{
          ...
          body:JSON.stringify({p_telegram_id:auth.user.id,p_first_name:auth.user.first_name,p_username:auth.user.username,p_game_code:gameCode,p_score:score,p_difficulty:difficulty,p_reaction_ms:reactionMs,p_false_starts:falseStarts,p_accuracy_percent:accuracy})
        });
        const text=await rpc.text(); if(!rpc.ok) return json({ok:false,error:"Could not save game result"},502);
        let result={};try{result=JSON.parse(text)}catch{} return json({ok:true,...result});
...
function render(){if(phase==="intro"){app.innerHTML='<section class="card"><div class="eyebrow">МИССИЯ · ПАМЯТЬ-СЕТКА</div><h1>Запомни рисунок</h1><p>Клетки загорятся. Затем повтори последовательность.</p><button id="start">Начать</button><button id="back" class="secondary">К миссиям</button></section>';start.onclick=begin;back.onclick=()=>location.href="/";return}if(phase==="memorize"||phase==="play"){const c=cfg(),active=phase==="memorize"?pattern:selected;app.innerHTML='<section class="card"><div class="eyebrow">'+(phase==="memorize"?"ЗАПОМИНАЙ":"ПОВТОРИ РИСУНОК")+'</div><h1>'+pattern.length+' клеток</h1><div class="memory-grid" id="grid" style="--n:'+c.n+'"></div><p>'+ (phase==="memorize"?"Смотри внимательно…":"Нажимай клетки по порядку")+'</p></section>';for(let i=0;i<c.n*c.n;i++){const b=document.createElement("button");b.className="cell"+(active.includes(i)?" active":"");b.disabled=phase!=="play";b.onclick=()=>pick(i);grid.appendChild(b)}return}const correct=selected.filter((x,i)=>x===pattern[i]).length,accuracy=Math.round(correct/pattern.length*100),score=Math.round(correct/pattern.length*700+d*100);app.innerHTML='<section class="card"><div class="eyebrow">РЕЗУЛЬТАТ · ПАМЯТЬ-СЕТКА</div><h1>'+score+'</h1><p>Точность: '+accuracy+'% · Сложность: '+d+'</p><div class="best">Правильно: <strong>'+correct+' / '+pattern.length+'</strong></div><button id="again">Следующий раунд</button><button id="home" class="secondary">К миссиям</button></section>';again.onclick=()=>{if(accuracy>=80&&d<6)d++;begin()};home.onclick=()=>location.href="/";save(score,accuracy)}
function begin(){saved=false;pattern=make();selected=[];phase="memorize";hap("light");render();setTimeout(()=>{phase="play";render()},cfg().ms)}
function pick(i){const k=selected.length;selected.push(i);if(pattern[k]!==i){hap("error");phase="result";render();return}hap("light");if(selected.length===pattern.length){hap("success");phase="result"}render()}
async function save(score,accuracy){if(saved)return;saved=true;const initData=tg?.initData;if(!initData)return;try{await fetch("/api/game-sessions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({initData,game_code:"memory_grid",score,difficulty:d,false_starts:0,accuracy_percent:accuracy})})}catch(_){saved=false}}
render();
</script></body></html>`;
...
function toHex(b){return[...new Uint8Array(b)].map(x=>x.toString(16).padStart(2,"0")).join("")}function constantTimeEqual(a,b){if(a.length!==b.length)return false;let x=0;for(let i=0;i<a.length;i++)x|=a.charCodeAt(i)^b.charCodeAt(i);return x===0}function json(data,status=200){return new Response(JSON.stringify(data),{status,headers:{"content-type":"application/json; charset=UTF-8","cache-control":"no-store","x-content-type-options":"nosniff"}})}
